...
class DX_Auth {
    ...
    function is_org_type_available($org_type, $id1) {  // Load Models
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_org_type($org_type, $id1);
        return $users->num_rows() == 0;
    }

    function is_zone_available($zone, $id1) {  // Load Models
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_zone($zone, $id1);
        return $users->num_rows() == 0;
    }
  ...
  function org_u1_available($username,$id1){
   ...
   function org_u1_available_person($person_number,$id1){
    ...
    function register($user_type, $emp_name, $username, $password, $email, $sale_prev = NULL, $engineering_prev = NULL, $commercial_prev = NULL, $hr_prev = NULL, $configuration_prev = NULL) {
        ...
        $new_user = array(
            ...
            'email' => $email,
            'sale_prev' => $sale_prev,
            'engineering_prev' => $engineering_prev,
            'commercial_prev' => $commercial_prev,
            'hr_prev' => $hr_prev,
            'configuration_prev' => $configuration_prev,
