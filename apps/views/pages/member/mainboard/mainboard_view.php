...
$query8=$this->db->query("select * from member_previlige where user_type='".$this->session->userdata("user_type")."'");
 foreach($query8->result() as $info8):
  endforeach;
    if ($info8->mainboard_access_main == '1'): 
?>
<style>
...
<h3 class="content_edit">Member Control Panel  Mainboard
...
<?php $query=$this->db->query("select * from article_category where org_id='".$this->session->userdata("member_org")."'");?>
 <?php echo form_open("org_member/info/viewed_mainboard"); ?>
...
<?php
/*$query1 = $this->db->query("select * from member_post where org_id='" . $this->session->userdata('member_org') . "'order by id desc");*/
$i = 1;
    ...
    if (date("Y-m-d") < $post_info->expire_date):
       ...
       $query850 = $this->db->query("select category_name from article_category where id='" . $post_info->article_category . "'");
        ...
        <span style="color:#0077D6" >
		  <a href="<?php echo base_url(); ?>org_member/info/view_mainboard_post_detail/<?php echo $post_info->id; ?>">
       ...
       <span style="color:black; font-size:12px" > <?php //echo substr("$post_info->text", 0, 50); ?><span>
        ...
        <span style="color:#689102; font-size:9px" >
          ...
          &nbsp;&nbsp;&nbsp; <b>Importance:</b> <?php echo $post_info->importance; ?>
           &nbsp;&nbsp;&nbsp; <b>Category:</b> 
		   <a href="<?php echo base_url(); ?>org_member/info/mainboard_viewed/<?php echo $post_info->article_category; ?>">
               <?php echo  $category_name; ?></a>		   
		   
          </span>
       <span id="n" style="float:right; padding-left:25px; width:300px;">
            <?php if($info8->mainboard_manually_archive=='1'):?>
            <a  href="<?php echo base_url() ?>org_member/info/archive_article/<?php echo $post_info->id; ?>" title="">
              <button class="a" name="archive"  value="archive">Archive</button>
            </a>
            <?php endif;?>
           </span>
            </div>
