<h3 class="content_edit">Admin Control Panel Modify Cost Seetings </h2>
...
<div class="infobox" style="width: 550px; margin-bottom: 20px; font-size: 12px; -moz-border-radius: 8px 8px 8px 8px;">
    ...
    <select name="package_name" style="width:150px" >
        <option value="">Select</option>
        <?php foreach ($query->result() as $info): ?>
          <option <?php if ($org_info->package_name == $info->id): ?> selected="selected" value="<?php echo $org_info->package_name; ?>" <?php else: ?> value="<?php echo $info->id; ?>"<?php endif; ?>><?php echo $info->package_name; ?></option>         
        <?php endforeach; ?>
    </select>
