    ...
    <div class="infobox_left" style="width: 900px;">
        <table width="900" cellspacing="1" align="center" cellpadding="2" border="0" id="theBody" style="margin-left:40px; margin-top:10px">
          ...
          <?php   $query = $this->db->query("select * from letter_header where org_id='" .$this->session->userdata('user_id') . "'and member_id=0");?>
                    ...
                    <td width="45%">
                       <select name="letter_header" style="width:190px">
...
<!--                        <input name="letter_title" style="width:200px" class="" type="text">
          ...
          <?php   $query1 = $this->db->query("select * from letter_footer where org_id='" .$this->session->userdata('user_id') . "'and member_id=0");?>
                    ...
                    <td width="45%">
                       <select name="letter_footer" style="width:190px">
                        <option value="">Select</option>
            ...
            <script>
              function check()
				{
				   var myvar = document.getElementById("a").value;
				   //var c='member'+myvar;
            ...
            <ul>
                ...
                <?php foreach ($q->result() as $p): ?>
                    <li style="width:200px; float:left; position:relative">
                        <span style="width:130px; float:left; position:relative; left:7px; color:green; font-weight:bold"> <?php echo $p->group_name; ?></span>
                        <span style="width:20px; float:left; position:relative"> 
                        <input id="a" type="checkbox" id="a"  name="checkbox[]" onClick="return check();"  value="<?php echo $p->id; ?>" />
                        </span>
                <?php $q8 = $this->db->query("select * from member where member_group='" . $p->id . "'"); ?>
                 <?php foreach ($q8->result() as $p8): ?>
                 <p style="display:none" id="member<?php echo $p->id;?>">
                     <span style="width:130px; float:left; position:relative; left:7px;"> <?php echo $p8->name; ?></span>
                     <span style="width:20px; float:left; position:relative; "> 
                     <input id="a" type="checkbox"  name="checkbox[]"  value="<?php echo $p8->id; ?>" />
                    </span>
                 <p>
                  <?php endforeach; ?>
                    </li>
